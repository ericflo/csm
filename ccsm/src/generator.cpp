...
void Generator::set_enable_advanced_context(bool enable) {
    enable_advanced_context_ = enable;
    
    // Create context manager if enabled and not already created
    if (enable && !context_manager_ && text_tokenizer_) {
        context_manager_ = std::make_shared<ContextManager>(text_tokenizer_);
        CCSM_INFO("Created advanced context manager");
    }
}

bool Generator::is_advanced_context_enabled() const {
    return enable_advanced_context_;
}

std::shared_ptr<ContextManager> Generator::context_manager() {
    // Create if not exists and we have a tokenizer
    if (!context_manager_ && text_tokenizer_) {
        context_manager_ = std::make_shared<ContextManager>(text_tokenizer_);
        CCSM_INFO("Lazily created context manager");
    }
    
    return context_manager_;
}

std::vector<float> Generator::generate_speech_with_context_manager(
    const std::string& text,
    int speaker_id,
    const GenerationOptions& options,
    std::function<void(int, int)> progress_callback) {
    
    if (!enable_advanced_context_ || !context_manager_) {
        CCSM_WARNING("Advanced context management not enabled, falling back to standard generation");
        return generate_speech(text, speaker_id, {}, options, progress_callback);
    }
    
    // Add current text as a new segment
    Segment segment;
    segment.text = text;
    ...
    enhanced.text = text;
    enhanced.speaker_id = speaker_id;
    enhanced.timestamp = std::chrono::duration_cast<std::chrono::milliseconds>(
        std::chrono::system_clock::now().time_since_epoch()).count();
    enhanced.importance_score = 1.0f; // Current segment gets full importance
    
    // Add to context manager
    context_manager_->add_enhanced_segment(enhanced);
    
    // Get tokens and positions from context manager
    std::vector<int> context_tokens = context_manager_->get_context_tokens();
    std::vector<int> positions = context_manager_->get_context_positions();
    
    // Limit token sequence length if necessary
    if (context_tokens.size() > static_cast<size_t>(max_text_tokens_)) {
        CCSM_WARNING("Context tokens exceed max_text_tokens, pruning context");
        
        // Prune context
        context_manager_->prune_context();
        
        // Get updated tokens and positions
        context_tokens = context_manager_->get_context_tokens();
        positions = context_manager_->get_context_positions();
    }
    
    // Prepare for generation
    CCSM_INFO("Generating speech with ", context_tokens.size(), " context tokens");
    
    // Reset model caches
    model_->reset_caches();
    
    // Copy options for potential modifications
    GenerationOptions working_options = options;
